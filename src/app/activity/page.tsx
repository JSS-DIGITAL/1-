...
function DayList({ date, entries }: { date: string; entries: ActivityEntry[] }) {
  const { removeActivityEntry } = useApp();
  ...
  return (
    <Card>
      ...
      {entries.length === 0 ? (
        <p className="mt-2 text-[0.875rem] text-muted">Nothing logged yet. Even ten minutes counts.</p>
      ) : (
...
function TrendsCard() {
  const { activityEntries } = useApp();
  const streak = activityStreakDays(activityEntries);
  const last7 = useMemo(() => {
